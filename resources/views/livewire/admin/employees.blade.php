<div>
  ...
  <div class="mb-4 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
    ...
    <div class="col-span-1 flex flex-col gap-2 sm:col-span-2 lg:col-span-3 lg:flex-row lg:items-center">
      <x-input type="text" class="w-full lg:w-72" name="search" id="seacrh" wire:model="search"
        placeholder="{{ __('Search') }}" />
      <div class="flex w-full gap-2 lg:w-auto">
        <x-button class="flex w-full justify-center sm:w-32" type="button" wire:click="$refresh" wire:loading.attr="disabled">
          {{ __('Search') }}
        </x-button>
        @if ($search)
          <x-secondary-button class="flex w-full justify-center sm:w-32" type="button" wire:click="$set('search', '')"
